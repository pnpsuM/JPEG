...
void Block() {
    ...
    printf("Blocked\n");
    for (int v = 0; v < BLK_SIZE; v++) {
        for (int u = 0; u < BLK_SIZE; u++)
            printf("%d\t", block[0][0][v][u]);
        printf("\n\n");
    }
}
void DCT() {
                ...
                for (u = 0; u < BLK_SIZE; u++) {
                    ...
                    for (y = 0; y < BLK_SIZE; y++) {
                        for (x = 0; x < BLK_SIZE; x++)
                            block_flt[i][j][v][u] += 0.25 * cv * cu * (float)block[i][j][y][x] * cos((((2 * y + 1) * v * M_PI)) / 16) * cos((((2 * x + 1) * u * M_PI)) / 16);
                    }
                }
            }
        }
...
void Quant() {
    ...
    for (v = 0; v < BLK_SIZE; v++) {
        for (u = 0; u < BLK_SIZE; u++)
            printf("%d\t", (char)block_flt[0][0][v][u]);
        printf("\n\n");
    }
}
void ZigZag() {
...
int main() {
    FILE* raw = NULL;
    FILE* out = NULL;
    fopen_s(&raw, "C:\\Users\\deep\\Desktop\\JunYoung\\Code\\JPEG\\lena.raw", "r");
    fopen_s(&out, "C:\\Users\\deep\\Desktop\\JunYoung\\Code\\JPEG\\lena_test.raw", "w");
    ...
    fwrite(block_flt, 1, PXL_SIZE * PXL_SIZE, out);
